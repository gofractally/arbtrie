...
namespace psitri
{
     ...
     public:
      ...
      struct subtree_sizer
      {
         ...
         bool fits_in_leaf() const
         {
            ...
            uint32_t avail = leaf_node::max_leaf_size - meta;
            return data <= avail;
         }
      };
            ...
            case node_type::leaf:
            {
               ...
               for (uint16_t i = 0; i < nb; ++i)
               {
                  ...
                  if (val.is_view() && !val.view().empty())
                     ...
                     out.value_data_size += sizeof(tree_id);
                  else if (val.is_value_node())
                     out.add_value_node(val.value_address());
                  if (out.overflow)
                     return;
               }
               out.count += nb;
                                           ...
                                           const branch_set&         sub_branches)
   {
      ...
      else if constexpr (mode.is_shared())
      {
	   ...
	   template <upsert_mode mode, any_inner_node_type InnerNodeType>
	   ptr_address tree_context::try_collapse_single_child_node(
	      ...
	      switch (node_type(child_ref->type()))
	      {
	         ...
	         case node_type::inner:
	         {
	            ...
	            if constexpr (is_inner_node<InnerNodeType>)
	            {
	               ...
	               {
	                  result_addr =
	                      _session.alloc<inner_node>(parent_hint, child_ptr, range, freq, _root_version);
	               }
	            }
	            else
	            {
	               ...
	               {
	                  result_addr = _session.alloc<inner_prefix_node>(
	                      parent_hint, child_ptr, key_view(prefix_buf, pfx.size()), range, freq,
	                      _root_version);
	               }
	            }
	            _session.release(child_addr);
	         ...
	         case node_type::inner_prefix:
	         {
	            ...
	            if constexpr (is_inner_node<InnerNodeType>)
	            {
	               ...
	               {
	                  result_addr =
	                      _session.alloc<inner_prefix_node>(parent_hint, ipn_ref.obj(), ipn_ref->prefix());
       ...
       ptr_address               child_addr)
   {
      ...
      auto try_pair = [&](branch_number left, bool current_is_left) -> ptr_address
      {
         ...
         else if constexpr (mode.is_shared())
         {
            ...
            if constexpr (is_inner_node<InnerNodeType>)
               ...
               result_addr = _session.alloc<InnerNodeType>(parent_hint, in->prefix(), in.obj(), update);
      ...
      {
         if (auto merged =
                 try_pair(branch_number(uint32_t(*br) - 1), false);
