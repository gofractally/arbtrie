...
TEST_CASE("tree_handle: works with OCC mode", "[tree_handle][occ]")
{
   ...
   auto tx = t.ses->start_transaction(0, tx_mode::occ);
   auto h  = tx.primary();

   auto v = h.get<std::string>("x");
   REQUIRE(v.has_value());
   CHECK(*v == "10");
   ...
   cur.seek("x");
   REQUIRE(!cur.is_end());
   CHECK(cur.value<std::string>().value_or("") == "11");
   cur.seek("z");
   REQUIRE(!cur.is_end());
   CHECK(cur.value<std::string>().value_or("") == "30");
}

// ════════════════════════════════════════════════════════════════════
// OCC remove_range throws
// ════════════════════════════════════════════════════════════════════

TEST_CASE("tree_handle: OCC remove_range throws logic_error", "[tree_handle][occ]")
{
   sal::set_current_thread_name("main");
   test_db t;
   ...
   // Via tree_handle
   {
      auto tx = t.ses->start_transaction(0, tx_mode::occ);
      auto h  = tx.primary();
      CHECK_THROWS_AS(h.remove_range("a", "c"), std::logic_error);
      tx.abort();
   }
   ...
   {
      auto tx = t.ses->start_transaction(0, tx_mode::occ);
      CHECK_THROWS_AS(tx.remove_range("a", "c"), std::logic_error);
      tx.abort();
   }
}
