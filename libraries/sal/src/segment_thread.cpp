...
#include <sal/debug.hpp>  // For SAL_* logging macros
...
namespace sal
{
   bool segment_thread::start()
   {
      ...
      if (already_running)
      {
         ...
         if (pid > 0)
         {
            ...
            {
               SAL_WARN(
                   "{} thread was running in process PID: {} (process no longer exists) - taking "
                   "over",
                   _thread_name, pid);
            ...
            else if (process_running)
            {
               ...
               if (last_alive == new_last_alive)
               {
                  ...
                  SAL_WARN(
                      "{} thread in process PID: {} is not making progress (no heartbeat for {}ms) "
                      "- taking over",
                      _thread_name, pid, elapsed_time);
                  _thread_state->running.store(false, std::memory_order_release);
                  // Fall through to restart the thread
               }
               else
               {
                  ...
                  SAL_INFO("{} thread is already running in process PID: {} (and making progress)",
                           _thread_name, pid);
                  return false;
               }
            }
         ...
         else
         {
            ...
            SAL_WARN("{} thread has running=true but no PID - taking over", _thread_name);
            _thread_state->running.store(false, std::memory_order_release);
            // Fall through to restart the thread
         }
      }
      ...
      _thread = std::thread(
          [this]()
          {
             try
             {
                ...
                sal::thread_name(_thread_name.c_str());
                ...
                {
                   SAL_ERROR("Exception in {} thread: {}", _thread_name, e.what());
                }
                catch (...)
                {
                   SAL_ERROR("Unknown exception in {} thread", _thread_name);
                }
             }
             catch (...)
             {
                // Ensure we handle any errors in setup
                SAL_ERROR("Unknown exception in thread setup: {}", _thread_name);
             }
      ...
      SAL_INFO("Started {} thread", _thread_name);
      return true;
   }
   ...
   void segment_thread::stop()
   {
      if (_thread.joinable())
      {
         ...
         SAL_INFO("Stopped {} thread", _thread_name);
      }
   }
   ...
   bool segment_thread::yield(std::chrono::milliseconds time_ms)
   {
      try
      {
         ...
         {
            SAL_WARN("Thread '{}' yield() call interval ({}ms) exceeds heartbeat interval ({}ms)",
                     _thread_name, time_since_last_yield.count(),
                     _heartbeat_warning_threshold.count());
         }
      ...
      {
         SAL_ERROR("Exception in {} thread yield: {}", _thread_name, e.what());
         return false;  // Signal thread to exit on error
      }
      catch (...)
      {
         SAL_ERROR("Unknown exception in {} thread yield", _thread_name);
         return false;  // Signal thread to exit on error
      }
   }
   ...
   void segment_thread::set_current_thread_name(const char* name)
   {
      ...
      // used by debug macros. For that, call sal::thread_name() directly
