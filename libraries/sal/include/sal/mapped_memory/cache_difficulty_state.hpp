   ...
   namespace mapped_memory
   {
      ...
      struct cache_difficulty_state
      {
         ...
         void compactor_update_difficulty(time_point current_time)
         {
            ...
            uint64_t max_uint64         = uint64_t(-1);
            ...
            else
            {
               ...
               new_difficulty   = static_cast<uint32_t>(max_uint64 - new_gap);
               /**/
         ...
         bool should_cache(uint64_t random, uint32_t size_bytes) const noexcept
         {
            ...
            uint64_t clines = ucc::round_up_multiple<64>(size_bytes + 1) / 64;
            ...
            return random >= (uint64_t)adjusted_difficulty;
         }
