...
// Directly test the tournament implementation for 32-element arrays
TEST_CASE("Tournament implementation for 32-element arrays", "[sal][min_index_32][tournament]")
{
   ...
   SECTION("Known minimum at specific positions")
   {
      ...
      {
         values[0] = 500;
         REQUIRE(sal::find_min_index32_tournament(values) == 0);
      }

      SECTION("Minimum in middle")
      {
         values[15] = 500;
         REQUIRE(sal::find_min_index32_tournament(values) == 15);
      }

      SECTION("Minimum at end")
      {
         values[31] = 500;
         REQUIRE(sal::find_min_index32_tournament(values) == 31);
      }
   }
   ...
   SECTION("With random values")
   {
      ...
      for (int test = 0; test < 10; test++)
      {
         ...
         uint32_t actual_idx = sal::find_min_index32_tournament(values);
   ...
   SECTION("With duplicate minimum values")
   {
      ...
      uint32_t result = sal::find_min_index32_tournament(values);
...
// Directly test the tournament implementation for 64-element arrays
TEST_CASE("Tournament implementation for 64-element arrays", "[sal][min_index_64][tournament]")
{
   // Allocate aligned memory for test arrays
   ...
   SECTION("Known minimum at specific positions")
   {
      ...
      {
         values[0] = 500;
         REQUIRE(sal::find_min_index64_tournament(values) == 0);
      }

      SECTION("Minimum in first half")
      {
         values[15] = 500;
         REQUIRE(sal::find_min_index64_tournament(values) == 15);
      }

      SECTION("Minimum in second half")
      {
         values[48] = 500;
         REQUIRE(sal::find_min_index64_tournament(values) == 48);
      }

      SECTION("Minimum at end")
      {
         values[63] = 500;
         REQUIRE(sal::find_min_index64_tournament(values) == 63);
      }
   }
   ...
   SECTION("With random values")
   {
      ...
      for (int test = 0; test < 10; test++)
      {
         ...
         // Find using our implementation
         uint32_t actual_idx = sal::find_min_index64_tournament(values);
         uint16_t actual_min = values[actual_idx];
   ...
   SECTION("With duplicate minimum values")
   {
      ...
      // The implementation should return one of the minimum indices
      uint32_t result = sal::find_min_index64_tournament(values);
...
// Test the public interface function for 32-element arrays
TEST_CASE("Find minimum index in 32-element arrays", "[sal][min_index_32][api]")
{
   // Allocate aligned memory for test arrays
   uint16_t* values = aligned_alloc<uint16_t>(32, 128);
...
// Test the public interface function for 64-element arrays
TEST_CASE("Find minimum index in 64-element arrays", "[sal][min_index_64][api]")
{
   // Allocate aligned memory for test arrays
   uint16_t* values = aligned_alloc<uint16_t>(64, 128);
