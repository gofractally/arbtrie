...
and batch-size scaling relative to each other.
...
| RAM | 121 GB |
| OS | Ubuntu Linux 6.17.0-14-generic x86\_64 (Vultr cloud VM) |
| Compiler | Clang 20, C++20, `-O3 -flto -march=native` |
...
PsiTri write throughput is **unaffected or slightly boosted** by a concurrent reader across all
batch sizes — consistent with its lock-free MVCC. TidesDB shows severe writer degradation at
batch=1M (−36%), and fails correctness at batch ≥ 1K (see note below).
...
†TidesDB balance conservation fails at batch ≥ 1K under concurrent read load (observed balance
exceeds expected by 0.008–32% depending on batch size), indicating a transaction isolation bug when
large write batches overlap with concurrent readers. Only the batch=1 result is valid for TidesDB.
